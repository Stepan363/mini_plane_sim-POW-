...
if start == False:
    ...
    def main():
        ...
        DISPLAY=pygame.display.set_mode((1350,600),0,32)
        ...
        pygame.draw.rect(DISPLAY,GREEN,(100,50,1150,150))
        pygame.draw.rect(DISPLAY,RED,(100,250,1150,350))
        while start == False:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pos[0] >= 100 and pos[0] <= 1250 and pos[1] >= 50 and pos[1] <= 200:
                    start = True
                if pos[0] >= 100 and pos[0] <= 1250 and pos[1] >= 250 and pos[1] <= 600:
                    quit()
...
if start == True:
    ...
    def draw():
        ...
        global plane_angle, plane, plane_exploded, speed, game_over, altitude, a,b, bird, bullet, start
        plane_exploded.draw()
    ...
    tree_change()
    def update():
        global altitude, t0,i, bird, ax,ay,end, t2, looped, recorded_x, recorded_y
        global speed, tree, bird_kills, countt
        global plane
        global win
        global is_flight_attemped
        global flight_attemped
        global plane_angle
        global game_over, difficuly, start
        is_developer()
        if keyboard.i:
            difficuly = 2
        if keyboard.o:
        ...
        if keyboard.p:
            difficuly = 6
        #if keyboard.a and plane.x > 0 and plane.x < WIDTH:
        #    countt += 10
        #    plane.pos = plane.x - countt, plane.y
        #if keyboard.d and plane.x > 0 and plane.x < WIDTH:
        #    countt -= 10
        #   plane.pos = plane.x - countt, plane.y
        if tree.x < 20:
        ...
        if speed > 1500:
            plane.pos = plane.x + random.randint(1,5)+countt, plane.y - random.randint(1,5)
        if plane_angle > 0 and plane.y >= 0 and plane.y <= HEIGHT:
            plane.pos = plane.x+countt, plane.y - plane_angle*20
        ...
        t3 = time.time()
        at = t3 - t2
        if keyboard.space and at >1 and looped < 25:
            #ax,ay=(random.randint(1200,1350),random.randint(0,600))
            recorded_y = plane.y
            recorded_x = plane.x+countt
            bullet.pos = bird.x, plane.y
            looped = 1
        if looped > 0 and looped < 25 and at>1:
            recorded_x += 25
            bullet.pos = recorded_x, recorded_y
        if looped == 25:
            t2 = t3
            at = t3 - t2
            looped = 0
        ...
        bx,by=(int(plane.x+countt), int(plane.y))
        ...
        if bullet.colliderect(bird):
            ...
            bird.pos = int(ax + stepx*i), int(ay + stepy*i)
            bird_kills += 1
            bullet.pos = 50000, 50000
            looped = 0


        

# in case of removing start-exit, just change start = True
